<div class="overflow-x-auto">
                        ...
                        <template x-teleport="body">
                            <div x-show="open" x-cloak x-transition @click.outside="close()"
                                ...
                                <form class="w-full"
                                    ...
                                    action="{{ route($delete, $item) }}" method="post">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                                ...
                                @if (isset($show))
                                <a class="block px-4 py-2 hover:bg-gray-100" href="{{ route($show, $item) }}">Ver</a>
                                @endif
                            </div>
                        </template>
                    </div>
    ...
    <div class="md:hidden space-y-4">
        ...
        <div class="bg-white p-4 border border-gray-200 rounded-lg shadow-sm">
            @for ($i = 0; $i < count($fields); $i++) <div class="flex justify-between py-1">
