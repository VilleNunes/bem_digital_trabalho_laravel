...
<div class="space-y-5">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
            <x-input-label for="quantify" value="Quantidade" />
            <x-text-input id="quantify" class="block mt-1 w-full" type="number" min="1" name="quantify"
                :value="$quantify" />
            <x-input-error :messages="$errors->get('quantify')" class="mt-2" />
        </div>

        <div>
            <x-input-label for="amount" value="Valor (opcional)" />
            <x-text-input id="amount" class="block mt-1 w-full" type="number" step="0.01" min="0" name="amount"
                :value="$amount" />
